...
public class ShareServiceImpl implements ShareService {
    ...
    public TUser queryAuditById(Integer id, UserAuditDTO auditDto) {
        ...
        if(!AuditStatusEnum.PASS.equals(auditDto.getAuditStatusEnum())){
            ...
            // 创建消息对象
            UserAddBonusMsgDTO msgDTO = UserAddBonusMsgDTO.builder().userId(user.getId()).bonus(50).build();
            Message<UserAddBonusMsgDTO> message = MessageBuilder.withPayload(msgDTO).setHeader(RocketMQHeaders.TRANSACTION_ID, transactionId).build();
            this.messageSource.addBonusOutput().send(message);
        }else{
